...
<!--
	This stylesheet transforms an `umbraco-package.xml` file into the `umbraco-package.json`.
...
>
	...
	<xsl:template match="manifest">
		<xsl:value-of select="$objStart" />
			<xsl:value-of select="concat($quot, '$schema', $quot)" />
			<xsl:text>:</xsl:text>
			<xsl:value-of select="concat($quot, @schema, $quot, ',')" />

			<xsl:apply-templates select="name" mode="json" /><xsl:text>, </xsl:text>
			<xsl:apply-templates select="version" mode="json" /><xsl:text>, </xsl:text>

			<xsl:text>"extensions": [</xsl:text>
				<xsl:apply-templates select="extension" />
			<xsl:text>]</xsl:text>
			<xsl:if test="css or javascript">, </xsl:if>
	...
	<xsl:template match="propertyEditor | field | extension | property">
		<xsl:text>{</xsl:text>
	...
	<xsl:template match="properties[property]" mode="json">
		<xsl:apply-templates select="." mode="quoted-name" />: [<xsl:text />
	...
	<xsl:template match="*" mode="array">
		<xsl:apply-templates select="." mode="quoted-name" />
		<xsl:text>: [</xsl:text>
			<xsl:apply-templates select="*" mode="json" />
		<xsl:text>]</xsl:text>
	</xsl:template>
	...
	<xsl:template match="* | @*" mode="quoted-name">
